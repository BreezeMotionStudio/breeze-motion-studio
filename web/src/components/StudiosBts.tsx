...
export function StudiosBts({ s, projects }: Props) {
  type BtsItem = {
    url: string
    alt?: string
    projectTitle: string
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="aspect-[4/3] border border-dashed border-white/10 rounded-sm"
                />
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {items.map((item, i) => {
              ...
              return item.projectSlug ? (
                <Link key={i} href={`/projects/${item.projectSlug}`} className="block">
                  {inner}
                </Link>
              ) : (
                <div key={i}>{inner}</div>
